...
def main() -> int:
    ...
    if failed == total:
        status, status_text = 2, "CRITICAL - DNS not responding"
    elif failed > 0:
        status, status_text = 1, "WARNING - Some tests failed"
    elif avg_time > 1000:
        status, status_text = 1, "WARNING - DNS slow response"
    else:
        status, status_text = 0, "OK"

    print(
        f"{status} {SERVICE} response_time={avg_time}ms;500;1000 "
        f"Test: {successful}/{total} OK, avg time: {avg_time}ms - {status_text} "
