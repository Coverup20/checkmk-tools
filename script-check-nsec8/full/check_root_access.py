...
def main() -> int:
    ...
    if failed_logins >= 10:
        status, status_text = 2, f"CRITICAL - Too many failed attempts ({failed_logins})"
    elif failed_logins >= 5:
        status, status_text = 1, f"WARNING - Failed attempts: {failed_logins}"
    elif active_sessions > 2:
        status, status_text = 1, f"WARNING - Too many active root sessions: {active_sessions}"
    elif successful_logins > 0 or active_sessions > 0:
        status, status_text = 0, f"OK - Logins: {successful_logins}, Active sessions: {active_sessions}"
    else:
        status, status_text = 0, "OK - No recent access"
