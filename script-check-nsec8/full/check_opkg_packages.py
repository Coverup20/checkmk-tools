...
def main() -> int:
    if shutil.which("opkg") is None:
        print("2 OPKG.Packages - opkg not available")
        return 0
    ...
    if overlay_used_pct >= 95:
        status, status_text = 2, f"CRITICAL - /overlay space: {overlay_used_pct}%"
    elif overlay_used_pct >= 85:
        status, status_text = 1, f"WARNING - /overlay space: {overlay_used_pct}%"
    elif updates_available >= 10:
        status, status_text = 1, f"WARNING - {updates_available} updates available"
    elif last_update_age >= 30:
        status, status_text = 1, f"WARNING - Package list outdated ({last_update_age} days)"
    elif updates_available > 0:
        status, status_text = 0, f"OK - {updates_available} updates available"
    else:
        status, status_text = 0, f"OK - {installed_count} packages installed"
