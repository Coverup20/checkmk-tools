#!/usr/bin/env python3
"""check_stunnel_agent_tls_ns8.py - CheckMK Local Check + self-remediation
for Agent TLS wrapping via Stunnel, on NethServer 8 (Rocky/RHEL9-based, dnf).
...
it (dnf install stunnel, rebind the socket, generate a cert if needed,
...
stunnel.service: on Rocky/RHEL, that unit hardcodes a single config path
(/etc/stunnel/stunnel.conf) and fails outright with any other filename -
found live while rolling this out on rl94ns8/rl94ns81, where the native
unit failed with "Invalid configuration file name" against our
checkmk-agent.conf.
...
def install_stunnel() -> bool:
    code, _, _ = run_command(["dnf", "install", "-y", "stunnel"], timeout=120)
    ...
    # config file/port (this is what happened live on rl94ns8/rl94ns81).
...
def remediate() -> Optional[str]:
    ...
    if not is_stunnel_installed():
        if not shutil.which("dnf"):
            return "dnf not found (not a NethServer 8/Rocky system?)"
