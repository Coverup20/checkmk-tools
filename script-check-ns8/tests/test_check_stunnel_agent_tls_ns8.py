"""Tests for check_stunnel_agent_tls_ns8.py (NethServer 8 (Rocky/RHEL9) variant).
...
import check_stunnel_agent_tls_ns8 as mod
...
def test_stunnel_service_name_is_always_the_dedicated_unit():
    ...
    # on Rocky/RHEL that unit hardcodes /etc/stunnel/stunnel.conf as the
...
def test_install_stunnel_disables_native_unit(monkeypatch):
    ...
    assert ok is True
    assert ["dnf", "install", "-y", "stunnel"] in calls
...
def test_remediate_returns_error_when_dnf_missing(monkeypatch):
    monkeypatch.setattr(mod, "is_stunnel_installed", lambda: False)
    monkeypatch.setattr(mod.shutil, "which", lambda name: None)

    error = mod.remediate()

    assert error == "dnf not found (not a NethServer 8/Rocky system?)"
