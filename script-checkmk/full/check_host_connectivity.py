...
# On same L2 subnet: uses nmap ARP scan (works even when ICMP/TCP are blocked).
# On different subnet (cross-VLAN): uses nmap ICMP via --send-ip (layer-3, crosses routers).
#
...
def get_local_networks():
    """Return list of (network_int, mask_int) tuples for all local interfaces."""
    nets = []
    try:
        out = subprocess.run(
            ["ip", "-4", "addr", "show"],
