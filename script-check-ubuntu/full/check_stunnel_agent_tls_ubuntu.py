#!/usr/bin/env python3
"""check_stunnel_agent_tls_ubuntu.py - CheckMK Local Check + self-remediation
for Agent TLS wrapping via Stunnel, on Debian/Ubuntu (apt).
...
def remediate() -> Optional[str]:
    ...
    if not is_stunnel_installed():
        if not shutil.which("apt-get"):
            return "apt-get not found (not a Debian/Ubuntu system?)"
