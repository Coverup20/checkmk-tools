"""Tests for check_stunnel_agent_tls_ubuntu.py (Debian/Ubuntu variant).
...
import check_stunnel_agent_tls_ubuntu as mod
...
def test_remediate_returns_error_when_apt_get_missing(monkeypatch):
    ...
    assert error == "apt-get not found (not a Debian/Ubuntu system?)"
